...
def main():
    ...
    #Create data
    train_gen, val_gen = Data.create_data(G=4)

    #Create Model
    init_model = InceptionResNetV2(include_top=False,weights='imagenet',input_tensor=None,input_shape=None,pooling='avg',bottleneck=None,G=4,classes=nb_class)
    last = init_model.output
    x = Dense(nb_class, activation='softmax', name='predictions')(last)
    model = Model(init_model.input, x)
    ...
    #checkpointer = ModelCheckpoint(filepath='stochastic_depth_multigpu_cdiscount.hdf5', verbose=1, save_best_only=True)
    ...
    if not data_augmentation:
        ...
        history = parallel_model.fit_generator(train_gen,
                                    ...
                                    steps_per_epoch=train_gen.samples // (64 * G),  #num_val_images // batch_size,
                                    workers = 8,
                                    callbacks=[tensorboard])

    # Save model via the template model (which shares the same weights):
    model.save_weight(file_save)
